    ...
    def load_network(self, load_path, network, strict=True, submodule=None):
        ...
        for k, v in load_net.items():
            if k.startswith('module.'):
                ...
                load_net_clean[k] = v
        missing_keys, unexpected_keys = network.load_state_dict(load_net_clean, strict=strict)

        if missing_keys or unexpected_keys:
            import warnings
            warnings.warn(f"Warning: missing_keys: {missing_keys} and unexpected_keys: {unexpected_keys} in load_network()")
